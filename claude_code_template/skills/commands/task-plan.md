---
name: task-plan
description: 컨텍스트를 파악해 task-exec가 실행 가능한 TASK.md를 생성
allowed-tools: ["Read", "Glob", "Grep", "Bash", "Write"]
model: sonnet
---

# /task-plan — 작업 계획 커맨드

## 역할 정의

너는 workflow.md의 **계획 담당자(Phase 1)** 역할이다.

절대 금지:
- 코드 파일 수정 (Edit 툴 사용 금지)
- git commit, git push
- TASK.md 이외의 파일 Write

허용:
- Read, Glob, Grep (파일 탐색 및 읽기)
- Bash (git log / git diff 읽기 전용)
- Write (temp/TASK.md 생성만)

## 실행 절차

### 1. 목적 확인

$ARGUMENTS를 읽어 작업 목적을 파악한다.
비어있으면 목적을 질문한다.

### 2. 컨텍스트 수집 (읽기 전용)

다음 순서로 정보 수집:

#### 2-1. 관련 파일 탐색
- Glob / Grep으로 $ARGUMENTS 키워드와 관련된 파일 탐색
- 예: "Alert" → DashboardRecentAlerts.tsx, dashboard.ts, alert_log 모델 등

#### 2-2. 대상 파일 읽기
- 탐색된 파일들을 Read 툴로 읽어 현재 구현 상태 파악
- 병렬 Read 가능

#### 2-3. git 히스토리 확인 (선택적)
```bash
# 관련 파일의 최근 커밋만 확인 (읽기 전용)
git log --oneline -10 -- {관련파일경로}
git log --oneline -5   # 전체 최근 5건
```

수집 대상 정보:
- 현재 구현 방식 (코드 패턴)
- 최근 변경 이력 (커밋 메시지)
- 관련 타입/스키마 정의

### 3. TASK.md 생성

수집한 정보를 바탕으로 `temp/TASK.md`를 Write 툴로 생성.

#### TASK.md 필수 섹션

```markdown
# TASK: {작업 제목}

> 작성일: {날짜}
> 담당: FE/BE 실행자
> 브랜치: {현재 브랜치}
> 범위: {수정 대상 파일 목록}

## 배경
{현재 문제점 / 목적}

## 수정 대상
| 파일 | 경로 |
|------|------|
| ... | ... |

수정 불필요:
- {건드리지 않을 파일}

## 구체적 변경 내용

### 1. {변경 항목}
{코드 블록 또는 명확한 지시 사항}

### 2. {변경 항목}
...

## 최종 예상 결과
{변경 후 동작/외형 설명}

## 검증 기준
- [ ] 항목1
- [ ] 항목2
```

#### 작성 품질 기준

- `/task-exec`가 추가 판단 없이 그대로 실행 가능할 정도로 구체적
- 코드 블록은 before/after 형태로 명시
- 모호한 표현 금지 ("개선", "정리" → "A를 B로 교체", "X 클래스 제거")
- 수정 불필요 파일도 명시 (실행자의 범위 이탈 방지)

### 4. 완료 보고

```
"TASK.md 생성 완료: temp/TASK.md
 수정 대상: {파일 수}개 파일
 검증 기준: {항목 수}개 항목
 실행 방법: /task-exec temp/TASK.md"
```

코드 수정 없이 종료.
