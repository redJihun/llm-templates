...
작업 시작 전, 이전 실행의 RESULT.md를 초기화합니다.

1. `$ARGUMENTS` 에서 TASK.md 경로를 파악해 디렉토리 추출 (기본: `temp/`)
2. `{디렉토리}/RESULT.md` 파일을 Read 툴로 읽어봅니다.
3. 파일이 존재하고 내용이 있으면 Write 툴로 빈 파일로 덮어씁니다.
4. 초기화한 경우 "⚙ RESULT.md 초기화 완료" 출력, 없거나 비어있으면 생략.
...
RESULT.md 작성이 완료된 후, 완료된 TASK.md를 초기화합니다.

Write 툴로 TASK.md 경로에 빈 내용을 덮어씁니다:
- 경로: 1단계에서 읽었던 TASK.md 경로
- 내용: 빈 문자열

초기화 후 "⚙ TASK.md 초기화 완료" 출력.
